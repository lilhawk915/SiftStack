...
# Warren's delinquent-tax PDF uses Account Numbers (7-digit) — NOT
# parcel numbers — as the primary key per row. The auditor exposes a
# separate Account search at this endpoint (verified live 2026-06-16).
ACCOUNT_FORM_URL = f"{AUDITOR_BASE}/PropertySearch/Search/AccountNumber"
ACCOUNT_SEARCH_URL = (
    f"{AUDITOR_BASE}/PropertySearch/Search/PerformAccountSearch"
)
...
def lookup_property_by_parcel(
    ...
    Pass ``client`` to reuse a connection across many lookups (the
    tax-delinquent adapter does ~1,350 sequential lookups — one client
    saves the TCP handshake overhead per request).
    """
    if not parcel_number:
        return ParcelAddress()
    clean = re.sub(r"[\s\-]", "", parcel_number)
    if client is not None:
        return _do_parcel_lookup(client, clean)
    # Fresh client for one-off calls (tests, ad-hoc usage)
    with httpx.Client(
        follow_redirects=True,
        headers={"User-Agent": "Mozilla/5.0 SiftStack/1.0"},
        timeout=20,
    ) as c:
        return _do_parcel_lookup(c, clean)


def _do_parcel_lookup(client: httpx.Client, clean: str) -> ParcelAddress:
...
def lookup_property_by_account(
    account_number: str,
    client: httpx.Client | None = None,
) -> ParcelAddress:
    """Fetch the property address for a Warren County Account Number.

    Warren's delinquent-tax PDF uses 7-digit account numbers as the
    primary key per row (NOT parcel numbers). The auditor exposes a
    separate Account search at ``ACCOUNT_SEARCH_URL`` which uses the
    same TaxYearId state. Result-page HTML schema matches the parcel
    search, so we reuse ``_parse_address_block``.

    Pass ``client`` to reuse a connection across many lookups.
    """
    if not account_number:
        return ParcelAddress()
    clean = re.sub(r"[\s\-]", "", account_number)
    if client is not None:
        return _do_account_lookup(client, clean)
    with httpx.Client(
        follow_redirects=True,
        headers={"User-Agent": "Mozilla/5.0 SiftStack/1.0"},
        timeout=20,
    ) as c:
        return _do_account_lookup(c, clean)


def _do_account_lookup(client: httpx.Client, clean: str) -> ParcelAddress:
    tax_id = _resolve_tax_year_id(client)
    if not tax_id:
        return ParcelAddress()
    try:
        r = client.get(
            ACCOUNT_SEARCH_URL,
            params={"AccountNumber": clean, "TaxYearId": tax_id},
        )
        r.raise_for_status()
    except Exception:
        return ParcelAddress()

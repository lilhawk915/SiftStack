...
CASE_TYPE_VALUE = "CV-FORECLOSURES"

# Clark format: "2026 CVE 00729" — 4-digit year + CVE + 5-digit seq
CASE_NUMBER_RE = re.compile(r"\b\d{2}CV\d{4,6}\b")
...
class ReconCapture:
    ...
    parsed_cases: list[ClarkParsedCase] = field(default_factory=list)
...
def parse_case_detail_html(html: str) -> ClarkCaseDetail:
    """Parse Clark equivant CourtView case-detail page.
    ...
    """
    detail = ClarkCaseDetail()
    soup = BeautifulSoup(html, "html.parser")

    # Case number from h1/heading text
    m = re.search(r"\b(\d{2}CV\d{1,6})\b", html)
    if m:
...
def parse_results_html(html: str) -> list[ClarkParsedCase]:
    ...
    """
    soup = BeautifulSoup(html, "html.parser")
    cases: list[ClarkParsedCase] = []
    seen: set[str] = set()

    # Primary path: equivant grid cells. Pull the WHOLE row text so we
    # can filter to foreclosure cases (Clark's caseCd is just CIVIL —
    # specific action type lives in a later cell of the row).
    ...
    for td in soup.find_all("td", id=re.compile(r"grid~row-\d+~cell-3$")):
        ...
        if cn in seen:
            continue
        # Pull full row text and require "Foreclosure" appears somewhere
        tr = td.find_parent("tr")
        row_text = tr.get_text(" ", strip=True) if tr else text
        if not fc_re.search(row_text):
            continue
        seen.add(cn)
        cases.append(ClarkParsedCase(
            case_number=cn,
...
class ClarkScraper:
    """Playwright scraper for Clark County CourtView CV-FORECLOSURES."""
    ...
    async def run(self) -> list[CaseRecord]:
        self.log.info(
            f"ClarkScraper start | mode={self.mode} | "
    ...
    async def _solve_disclaimer_captcha(self, page: Page,
        ...
        """Solve Clark's CourtView disclaimer CAPTCHA + click I Accept.
    ...
    async def _try_solve_once(self, page: Page) -> bool:
        ...
        if not api_key:
            self.log.warning(
                "No 2Captcha API key — Clark disclaimer cannot be bypassed"
            )
        ...
        try:
            ...
            if await img_loc.count() == 0:
                self.log.warning("No captcha image on Clark landing")
        ...
        image_bytes = b""
        try:
            self.log.info("Fetching Clark CAPTCHA image ...")
            captcha_src = await img_loc.get_attribute("src")
            if captcha_src:
                # Use the CURRENT page URL as the base (not PORTAL_URL) —
                # PORTAL_URL may be a bare domain while the captcha src
                # ?x=... is meant to replace the current page's query.
                if captcha_src.startswith("?"):
                    base = page.url.rsplit("?", 1)[0]
                    full_url = base + captcha_src
                elif captcha_src.startswith("/"):
                    portal_host = "/".join(page.url.split("/")[:3])
    ...
    async def _set_case_type_filter(self, page: Page) -> None:
        ...
        # Clark caseCd has only broad case types (CIVIL, CRIMINAL, etc.) —
    ...
    async def _set_date_range(self, page: Page) -> None:
        # Clark uses Wicket-style nested field names same as probate
        for field, value in [

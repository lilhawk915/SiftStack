"""Clark County (Ohio) Probate Court — probate.clarkcountyohio.gov scraper.

SAME VENDOR family as Clark probate (PHP `recordSearch.php` system).
Agency ID = 1203 in the URL token.

Per H3 SOP H3-SOP-CCO-002:
  1. GET https://probate.clarkcountyohio.gov/recordSearch.php?k=<token>
...
Reuses Clark's case-detail parser since the vendor DOM is identical.
"""
...
# Clark's case-detail DOM is identical to Butler's (same PHP vendor),
# so we reuse Butler's parser directly. Aliased for readability.
from h3.parsers.butler_probate_case_detail import (
    ButlerProbateDetail as ClarkProbateDetail,
    parse_case_detail,
)
from h3.output_writers.probate_format import ProbateRecord


# ── Portal config (per H3 SOP H3-SOP-CCO-002) ──────────────────────────

PORTAL_URL = (
    "https://probate.clarkcountyohio.gov/recordSearch.php"
    "?k=searchForm1203"
)
CASE_TYPE_VALUE = "ESTATE"
PORTAL_HOST = "https://probate.clarkcountyohio.gov"
# Clark uses agency ID 1203 (Clark is 1203). The acceptAgreement URL
# pattern is the same — just with this token.

# Clark probate case-number format: E + YYYYNNNN (e.g. E20261216)
...
def _to_us_date(iso_date: str) -> str:
    """Convert YYYY-MM-DD → MM/DD/YYYY (Clark form likely expects US format)."""
    if not iso_date:
        return ""
    try:
...
class ReconCapture:
    ...
    parsed_cases: list[ClarkProbateCase] = field(default_factory=list)
...
def _detail_to_record(detail: ClarkProbateDetail) -> ProbateRecord:
    """Convert a Clark case-detail page into a ProbateRecord row.

    Clark gives us 11 of 12 columns directly from the case detail page.
    Only Fiduciary Email comes from an application PDF (deferred).

    Mapping (Clark → DM probate schema):
...
_CLARK_CASE_URL_RE = re.compile(
    r'/recordSearch\.php\?k=case1203[^"\']+',
)
_CLARK_DOCKET_URL_RE = re.compile(
    r'/recordSearch\.php\?k=docket1203[^"\']+',
)


@dataclass
class ClarkProbateCaseV2:
    """Richer per-case structure parsed from the results page table."""
    case_number: str
    decedent_name: str = ""
    date_filed: str = ""
    case_type: str = ""            # "Estate" etc.
    case_url: str = ""             # /recordSearch.php?k=case1203...
    docket_url: str = ""           # /recordSearch.php?k=docket1203...


def parse_results_html(html: str) -> list[ClarkProbateCase]:
    """Parse Clark probate results — Caselook PHP DOM.
    ...
    Confirmed structure (build 0.1.52 recon, 5/26/2026, 17 cases):
      <div class="record">
        ...
        <div class="caseInfo">
          ...
          <a href="/recordSearch.php?k=case1203..." class="caseLink ...">Case</a>
          <a href="/recordSearch.php?k=docket1203..." class="docketLink ...">Docket</a>
        </div>
      </div>
    """
    soup = BeautifulSoup(html, "html.parser")
    cases: list[ClarkProbateCase] = []
    seen: set[str] = set()
...
class ClarkProbateScraper:
    """Playwright scraper for Clark County Probate Court (PHP).
    ...
    async def run(self) -> None:
        self.log.info(
            f"ClarkProbateScraper start | mode={self.mode} | "
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                # Clark's Caselook portal only accepts one date per search.
                # Iterate each day in window; CAPTCHA per submit (~$0.0005/day).
                # Case-detail URLs are session-scoped — capture inline.
                if self.date_from:
                    try:
    ...
    async def _navigate_to_search(self, page: Page) -> None:
        ...
        if m:
            accept_url = "https://probate.clarkcountyohio.gov" + m.group(1)
    ...
    async def _solve_captcha(self, page: Page) -> None:
        """Capture Clark's CAPTCHA image, solve via 2Captcha, fill response.
        ...
        if not api_key:
            self.log.warning(
                "No 2Captcha API key — Clark search will fail without it"
            )
    ...
    async def _capture_details_for(
        ...
        """Per-day case-detail + docket capture. Tokens fresh from same day."""
        if not cases:
            return
        self.log.info(
        ...
        base_prefix = "https://probate.clarkcountyohio.gov"
        for i, case in enumerate(cases):
            cap = CaseDetailCapture(case_number=case.case_number)
            detail: ClarkProbateDetail | None = None
            # Open in fresh tab so URL tokens stay independent (Caselook
            ...
            if case.detail_url:
                ...
                try:
                    ...
                    try:
                        ...
                        # but Clark uses E-prefix format. Override with the
    ...
    async def _capture_results(self, page: Page) -> None:
        ...
        self.log.info(
            f"Parsed {len(cases)} Clark probate case numbers from results."
        )
